...
def get_noise_model(noise_type="gaussian,0,50"):
    ...
    if tokens[0] == "gaussian":
        ...
        def poisson_noise(img):
            ...
            # unique_length = len(np.unique(img_arr))
            # values = 2 ** np.ceil(np.log2(unique_length))
            # # Generating noise for each unique value in image.
            # out = np.random.poisson(img_arr * values) / float(values)
            imageplusshotnoise = np.random.poisson(lam=img_arr)
            poisson_img = np.clip(imageplusshotnoise, 0, 255).astype(np.uint8)
            return poisson_img
...
def get_args():
    ...
    parser.add_argument("--noise_model", type=str, default="gaussian,0,20",
...
def main():
    ...
    while True:
        image = np.ones((image_size, image_size, 3), dtype=np.uint8) * 127
        noisy_image = noise_model(image)
        cv2.imshow('noisy Image', noisy_image)
        # cv2.imwrite('poissonNoise.png', noisy_image)
